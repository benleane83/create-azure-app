...
app.http('listItems', {
  ...
  handler: async (
    ...
    const rows = await db
      .select()
      .from(items)
      .where(eq(items.userId, currentUser.id))
...
app.http('getItem', {
  ...
  handler: async (
    ...
    const [item] = await db
      .select()
      .from(items)
      .where(and(eq(items.id, id), eq(items.userId, currentUser.id)));
    if (!item) {
      return { status: 404, jsonBody: { error: 'Item not found' } };
    }
...
app.http('deleteItem', {
  ...
  handler: async (
    ...
    await db.delete(items).where(and(eq(items.id, id), eq(items.userId, currentUser.id)));
    return { status: 204 };
  },
});
`;
